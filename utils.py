...
def cal_params_flops(model, size, logger, input_channels=3):
    # 临时抑制thop模块的INFO输出
    thop_logger = logging.getLogger('thop')
    original_level = thop_logger.level
    ...
    try:
        ...
        # 确保输入tensor在与模型相同的设备上
        device = next(model.parameters()).device
        print(f"🔧 使用 {input_channels} 通道输入进行FLOPs计算...")
        input = torch.randn(1, input_channels, size, size).to(device)
        ...
        total = sum(p.numel() for p in model.parameters())
        print("Total params: %.2fM" % (total/1e6))
        logger.info(f'Total params: {total/1e6:.4f}M (FLOPs计算跳过)')
    finally:
        # 恢复原始日志级别
        thop_logger.setLevel(original_level)
...
def calculate_dice_only(pred, gt):
    """只计算Dice系数，跳过HD95计算以提高速度"""
    pred[pred > 0] = 1
    gt[gt > 0] = 1
    if pred.sum() > 0 and gt.sum() > 0:
        dice = metric.binary.dc(pred, gt)
...
def postprocess_segmentation(prediction, keep_largest_component=False, fill_holes=False, classes=None):
    """Validation-gated nnU-Net-style post-processing for label maps."""
    if not keep_largest_component and not fill_holes:
        return prediction
    try:
    ...
    out = prediction.copy()
    if classes is None:
        classes = [c for c in np.unique(prediction) if c != 0]
    for cls in classes:
        mask = prediction == cls
        if fill_holes:
...
def test_single_volume(image, label, net, classes, patch_size=[256, 256],
                    ...
                    network_type=None, postprocess_config=None):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    # 自动检测模型期望输入通道
    expected_in_chans = 1
    try:
        m = net.module if hasattr(net, 'module') else net
        if hasattr(m, 'patch_embed') and hasattr(m.patch_embed, 'proj') and hasattr(m.patch_embed.proj, 'in_channels'):
            expected_in_chans = int(m.patch_embed.proj.in_channels)
    except Exception:
        expected_in_chans = 1
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            # 根据期望通道数构建输入
            if expected_in_chans == 3:
                input = torch.from_numpy(slice).unsqueeze(0).float().cuda()  # (H, W) -> (1, H, W)
                input = input.repeat(3, 1, 1).unsqueeze(0)                 # -> (1,3,H,W)
            elif expected_in_chans == 1:
                ...
                input = input.repeat(expected_in_chans, 1, 1).unsqueeze(0)
    ...
    if postprocess_config is not None and getattr(postprocess_config, "postprocess_enabled", False):
        prediction = postprocess_segmentation(
            prediction,
            keep_largest_component=getattr(postprocess_config, "postprocess_keep_largest_component", False),
            fill_holes=getattr(postprocess_config, "postprocess_fill_holes", False),
...
def test_single_volume_dice_only(image, label, net, classes, patch_size=[256, 256], 
                               test_save_path=None, case=None, network_type=None):
    """只计算Dice指标的快速测试函数，跳过HD95计算和文件保存"""
    # 自动检测模型期望输入通道
    expected_in_chans = 1
    try:
        m = net.module if hasattr(net, 'module') else net
    ...
    dice_list = []
    for i in range(1, classes):
        dice_score = calculate_dice_only(prediction == i, label == i)
